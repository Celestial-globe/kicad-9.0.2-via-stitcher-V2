...
# ロギング設定
# logging.basicConfig(
#    filename=os.path.join(os.path.dirname(__file__), "via_stitcher.log"),
#    level=logging.DEBUG,
...
class ViaStitcher(pcbnew.ActionPlugin):
    def defaults(self):
        ...
        self.category = "配置"
        self.description = (
            "選択したゾーンにVIAを均等に配置し、オプションでグループ化します"
        )
        self.show_toolbar_button = True
        self.icon_file_name = os.path.join(
            os.path.dirname(__file__), "via-stitcher-icon.png"
        )
    ...
    def Run(self):
        try:
            ...
            if not selected_zones:
                wx.MessageBox(
                    "ゾーンが選択されていません。", "情報", wx.OK | wx.ICON_INFORMATION
                )
                return
            ...
            if dialog.ShowModal() == wx.ID_OK:
                ...
                logging.debug(
                    f"Settings: via_size={dialog.via_size_mm}mm, via_drill={dialog.via_drill_mm}mm, via_spacing={dialog.via_spacing_mm}mm, net_name={net_name}, group_vias={group_vias}, use_zone_net={use_zone_net}"
                )
                    ...
                    if isinstance(item, pcbnew.ZONE):
                        ...
                        if progress_dialog:
                            progress_dialog.Update(
                                idx,
                                f"ゾーン {idx+1}/{len(selected_items)} にVIAを配置中...",
                            )

                        # ゾーンのネット名を使用する場合、その都度ネットを取得
                        current_net_name = (
                            item.GetNetname() if use_zone_net else net_name
                        )
                        ...
                        if net_code is not None:
                            zone_vias = self.place_vias_in_zone(
                                board, item, via_size, via_drill, via_spacing, net_code
                            )
                            # 追加したVIAを全体のリストに追加
                            ...
                            wx.MessageBox(
                                f"ネット '{current_net_name}' が見つかりません。",
                                "エラー",
                                wx.OK | wx.ICON_ERROR,
                            )
                ...
                if vias_count > 0:
                        ...
                        if use_zone_net:
                            ...
                            for net_name, vias in net_vias_map.items():
                                if vias:  # 空のリストでなければグループ化
                                    ...
                                    logging.debug(
                                        f"Created group for net {net_name} with {len(vias)} vias"
                                    )
                        else:
                            ...
                            logging.debug(
                                f"Created single group with {len(added_vias)} vias"
                            )
                    ...
                    pcbnew.Refresh()
                    message = (
                        f"VIAの配置が完了しました。{vias_count}個のVIAを追加しました。"
                    )
                    if group_vias:
            ...
            if "progress_dialog" in locals() and progress_dialog:
                progress_dialog.Destroy()
            wx.MessageBox(
                f"エラーが発生しました: {str(e)}", "エラー", wx.OK | wx.ICON_ERROR
            ).Destroy()
        ...
        try:
            ...
            if hasattr(netinfo, "FindNet"):
                ...
                if net:
                    ...
                    logging.debug(
                        f"Found net using FindNet: {net_name}, code: {net_code}"
                    )
            # アプローチ2: GetNetItem
                ...
                if net:
                    net_code = net.GetNetCode()
                    logging.debug(
                        f"Found net using GetNetItem: {net_name}, code: {net_code}"
                    )
            # アプローチ3: 全ネット列挙
            else:
                for net_item in netinfo.NetsByNetcode():
                    ...
                    if netname == net_name:
                        ...
                        logging.debug(
                            f"Found net in list: {net_name}, code: {net_code}"
                        )
                        break
            ...
            if net_code is None and hasattr(board, "GetNetsByName"):
                ...
                if net_name in nets_by_name:
                    ...
                    logging.debug(
                        f"Found net using GetNetsByName: {net_name}, code: {net_code}"
                    )
    ...
    def place_vias_in_zone(
        self, board, zone, via_size, via_drill, via_spacing, net_code
    ):
        ...
        try:
            ...
            logging.debug(
                f"Collected {len(existing_via_positions)} existing via positions"
            )
            ...
            # デバッグ情報をログに記録
            logging.debug(
                f"Zone bounding box: ({x_start}, {y_start}) to ({x_end}, {y_end})"
            )
            logging.debug(
                f"Via spacing: {via_spacing}, X count: {x_count}, Y count: {y_count}"
            )
            ...
            if hasattr(netinfo, "GetNetItem"):
                ...
                if net:
                    logging.debug(
                        f"Using net code {net_code} for net '{net.GetNetname()}'"
                    )
        ...
        for item in board.GetTracks():
            if type(item) == pcbnew.PCB_VIA:
                ...
                for dx in range(-1, 2):
                    for dy in range(-1, 2):
                        positions.add(
                            (x_grid + dx * tolerance, y_grid + dy * tolerance)
                        )
        return positions
            ...
            try:
                ...
                if hasattr(pcbnew, "PCB_GROUP"):  # PCB_GROUP クラスが存在するか確認
                    ...
                    if hasattr(pcbnew, "PCBNEW_SELECTION"):
                        ...
                        board.Add(selection)
                        logging.debug(
                            f"Created selection group '{group_name}' with {len(vias)} vias"
                        )
                        return True
    ...
    def save_settings(
        ...
        try:
            ...
            settings_path = os.path.join(
                os.path.dirname(__file__), "via_stitcher_settings.json"
            )
...
class ViaStitcherDialog(wx.Dialog):
    def __init__(self, parent, selected_zone_net=""):
        wx.Dialog.__init__(
            self, parent, id=wx.ID_ANY, title="VIAステッチ設定", size=(300, 380)
        )
        ...
        self.use_zone_net = (
            False  # 新しいオプション: ゾーンのネット名を使用するかどうか
        )
        ...
        if settings:
            ...
            self.use_zone_net = settings.get(
                "use_zone_net", self.use_zone_net
            )  # 新しい設定を読み込む
        ...
        self.save_checkbox = wx.CheckBox(
            self, label="この設定を次回のデフォルトとして保存する"
        )
    ...
    def on_ok(self, event):
        try:
            ...
            self.use_zone_net = (
                self.zone_net_checkbox.GetValue()
            )  # ゾーンのネット名を使用するかどうか

            if (
                self.via_size_mm <= 0
                or self.via_drill_mm <= 0
                or self.via_spacing_mm <= 0
            ):
                wx.MessageBox(
                    "値は正の数を入力してください。", "エラー", wx.OK | wx.ICON_ERROR
                )
                return

            if self.via_drill_mm >= self.via_size_mm:
                wx.MessageBox(
                    "ドリルサイズはVIAサイズより小さくしてください。",
                    "エラー",
                    wx.OK | wx.ICON_ERROR,
                )
                return
            ...
            wx.MessageBox(
                "有効な数値を入力してください。", "エラー", wx.OK | wx.ICON_ERROR
            )
